...
class ParkingLot:
    ...
    def status(self):
        free_car = sum(
            1 for s in self._spots if s.is_free() and s.get_allowed_type() == "Car"
        )

        free_moto = sum(
            1 for s in self._spots if s.is_free() and s.get_allowed_type() == "Motorcycle"
        )
        return free_car, free_moto
